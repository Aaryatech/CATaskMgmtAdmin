	...
	display: none;
				...
				<div class="card">
					<div class="card-header header-elements-inline">
						<h5 class="card-title">Completed Task ${reportTitle}- <b>${empData.empName}</b></h5>

					</div>
					<div class="card-body">
						<form
							action="${pageContext.request.contextPath}/getCompTaskVariation/${reportType}"
									...
									<c:if test="${reportType==2}">
										<c:forEach items="${taskReportList}" var="taskRep"
											...
											<tr>
												...
												<c:set var="variance" value="0"></c:set>

												<c:if test="${empData.empType==5}">
													<td>${taskRep.empBudHr}</td>
													<c:set var="variance" value="${taskRep.empHrVariance}"></c:set>
													<c:set var="varianceInt" value="${taskRep.hourDiffEmp}"></c:set>

												</c:if>
												<c:if test="${empData.empType==3}">
													<td>${taskRep.mngrBudHr}</td>
													<c:set var="variance" value="${taskRep.mngHrVariance}"></c:set>
													<c:set var="varianceInt" value="${taskRep.hourDiffMng}"></c:set>

												</c:if>
												<td>${taskRep.workHours}</td>
												<c:set var="colorStyle" value=""></c:set>
												<c:if test="${varianceInt < 0}">
													<c:set var="colorStyle" value="style=background-color:red"></c:set>
												</c:if>

												<c:if test="${varianceInt > 0}">
													<c:set var="colorStyle"
														value="style=background-color:green"></c:set>
												</c:if>
												<td ${colorStyle}>${variance}</td>
												<td>${taskRep.delLink}</td>
											</tr>
										</c:forEach>
									</c:if>
									...
									<c:if test="${reportType==1}">
										<c:forEach items="${taskReportList}" var="taskRep"
											...
											<tr>
												...
												<td ${colorStyle}>${taskRep.dateDiff}</td>
												<td>${taskRep.delLink}</td>
											</tr>
										</c:forEach>
