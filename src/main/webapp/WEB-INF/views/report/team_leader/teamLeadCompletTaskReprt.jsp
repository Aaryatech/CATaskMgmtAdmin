...
.datatable-header {
	display: none;
				...
				<div class="card">
					...
					<div class="card-body">
						<form
							action="${pageContext.request.contextPath}/showCompTaskReportForTeamLead"
							...
							<div class="form-group row">
								...
								<div class="col-lg-1"></div>
								<a
									href="${pageContext.request.contextPath}/showTLTaskCompltRep?fromDate=${fromDate}&toDate=${toDate}"><button
							...
							<table class="table table-bordered table-hover datatable-highlight1 datatable-button-html5-basic  datatable-button-print-columns1" id="capTable">
								<thead>
									<tr class="bg-blue">
										...
										<th>TL Hrs total</th>
										<th>TL Hrs for the selected period</th> 
									</tr>
								</thead>
								<tbody>
									...
									<c:forEach items="${cmpTaskList}" var="cmpTaskList"
										...
										<tr>
											...
											<td>${cmpTaskList.clientName}</td>
											 <td>${cmpTaskList.service}</td>  
											 <td>${cmpTaskList.activity}</td>
											 <td>${cmpTaskList.taskPeriodicity}</td>
											<td>${cmpTaskList.executionPartner}</td>
											<td>${cmpTaskList.managerName}</td>
											<td>${cmpTaskList.teamLeadName}</td>
											<td>${cmpTaskList.dueDate}</td>
											<td>${cmpTaskList.completionDate}</td>
											<td>${cmpTaskList.empBudHr}</td>
											<td>${cmpTaskList.workHours}</td>
											<td>${cmpTaskList.tlTotalHrs}</td>
											<td>${cmpTaskList.tlPeriodHrs}</td> 
										</tr>
									</c:forEach>
								</tbody>
							</table>
